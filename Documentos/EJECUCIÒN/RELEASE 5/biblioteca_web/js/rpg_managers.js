...
StorageManager.saveToWebStorage = function(savefileId, json) {
	...
	if(key == 'RPG Global' || key == 'RPG File'+savefileId){
		if(data != 'null' && data != '' ){
			key_post=key;
			data_post=data;
			save_id=savefileId;
						
			jQuery.post('/biblioteca_web/php/include/save.php', { key_post:key_post, data_post:data_post, save_id:save_id });
		}		
	}
...
StorageManager.loadFromWebStorage = function(savefileId) {
	...
	var data = $.ajax({
		method: 'POST',
		url: '/biblioteca_web/php/include/load.php',
		dataType: 'text',
		data: { savefileId:savefileId, key_post:key},	
		async: false 
	}).responseText;
